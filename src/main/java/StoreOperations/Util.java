...
/**
 * Simple CLI for store operations: register new employee or login to a register (POS).
...
public class Util {
	...
	private static void registerNewCashier(Scanner scanner){
		try{
			...
			if(match == null){
				System.out.println("No employee with ID " + id + " found in employees.txt. Cannot grant register access.");
				return;
			}

			System.out.println("Found employee: " + match.getFName() + " " + match.getLName());
			if(match.getUsername() != null && !match.getUsername().isEmpty()){
				System.out.print("This employee already has a username (" + match.getUsername() + "). Overwrite? (Y/N): ");
			...
			// persist credentials only to registerEmployees.txt (do not modify employees.txt)
			db.writeRegisterCredentials(match);
			System.out.println("Granted register access to employee ID: " + match.getID() + ". Credentials saved to registerEmployees.txt");
	...
	private static void loginToRegister(Scanner scanner){
		...
		// Prevent overwriting an already-active logged-in employee on the shared session
		StorePOS current = Session.getCurrentPOS();
		if (current != null && current.getLoggedInEmployee() != null) {
			HR.BaseEmployee active = current.getLoggedInEmployee();
			System.out.println("A user is already logged in on the active register: " + active.getFName() + " " + active.getLName() + ". Please logout first.");
			return;
		}

		// Use existing POS in session if present (and not logged in), otherwise create a new POS
